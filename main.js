...
document.getElementById("esboco").innerHTML = 'Esboco a ser desenho: ' + sketch;
...
function draw(){
  ...
  if (drawnSketch == sketch){
    answerHolder = "set";
    score++;
    document.getElementById("score").innerHTML = 'Pontuacao: ' + score;
  }
}
...
function gotResult(error, results){
  if(error){
    console.error(error);
       ...
       document.getElementById("label").innerHTML = "Seu esboco: "  + drawnSkecth.replace("_", " ");
...
function checkSketch(){
  timeCounter++;
  document.getElementById("time").innerHTML = "Tempo:  " + timeCounter;
  if (timeCounter > 400){
    timeConter = 0;
